...
"""
        ...
        result_contract = """Result contract:
{"outcome":"finished","detail":"batch rationale","evidence":{"synthesis_requests":[{"schema_version":1,"type":"new_concept|controlled_improvement","lane":"new_concept|improvement","action":"new|revise","source_experiment_id":null,"controlled_change":"one change, required for controlled_improvement","thesis":"falsifiable research thesis","archetype":"trend|mean_reversion|breakout|other","target_regime":"...","failure_regime":"...","falsifiability_criteria":"what would reject this idea","entry_rule_summary":"precise entry logic","why_this_now":"evidence-backed reason for choosing it now","expected_edge_type":"continuation|reversion|breakout|volatility|other","strategy_name":"ExistingOrConcreteStrategy","hypothesis":"...","edge_thesis":"why edge should persist","change_scope":"new_entry|entry_changed|exit_only|sizing_only|risk_only|refactor","priority":20,"n_simulations":2000,"random_seed":42,"routes":[{"exchange":"Binance Perpetual Futures","symbol":"BTC-USDT","timeframe":"1h","start_date":"YYYY-MM-DD","finish_date":"YYYY-MM-DD"}]}]}}

Return exactly context.generate_limit requests (normally 25) as one coherent cohort. Think in three bounded modes: reserve at least lane_policy.minimum_new_concepts for genuinely new concepts; use at most lane_policy.maximum_improvements for controlled improvements; and use failure_diagnoses to design counter-experiments against diagnosed strategy failures. Improvements must make exactly one controlled change to an inconclusive or failed source. Backfill unused improvement capacity with new concepts. Use canonical evidence, archetype_coverage, forbidden_actions, and untrusted advisory_memory to diversify ideas and avoid cosmetic variants. Every proposal must contain all typed fields; the deterministic laboratory validates exact count, lane allocation, fingerprints, source verdict, revision depth, significance, and capacity. Never return, revise, archive, supersede, or otherwise touch hpo_candidate or paper_trade_candidate records; they are omitted and promotion-locked. Infrastructure failures are retries, not revisions. New or changed entry rules must use new_entry or entry_changed. Exit/sizing/risk-only changes retain exact entry rule. Never invent completed results. Cohort ID and slots are assigned by laboratory. The agent proposes only; it cannot change job state, create evidence, bypass gates, or override SQLite authority."""
    else:
        result_contract = """Result contract:
