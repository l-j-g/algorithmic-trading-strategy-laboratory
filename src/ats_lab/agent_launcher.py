...
Return one evaluation for every execution exactly once. Each execution contains only canonical normalized evidence records. Analyze those records directly; do not create another metric schema, request raw evidence, call tools, or repeat backtests. Deterministic verdicts already present in normalized evidence are authoritative.""" + hpo_instruction + """ Return synthesis_requests as an empty list. Ordinary evaluation, HPO analysis, and synthesis are separate task types.
